      ...
      {levels.map((level) => (
        ...
        >
          <span className="flex flex-col gap-0.5">
            ...
            <span>{getLevelTypeLabel(level.quiz_type)}</span>
          </span>
          <span className="text-[10px] font-bold">
            {level.is_completed ? 'Selesai · Ulangi' : level.is_locked ? `Terkunci — selesaikan Level ${Math.max(1, level.level_number - 1)} dulu` : 'Mulai Level'}
          </span>
        </button>
      ))}
    </div>
